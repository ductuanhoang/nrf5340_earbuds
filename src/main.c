...
{
	// TODO Tuan need change to set maximum volume and assigned to AUDIO_CH_L
	// Done
...
void main(void)
{
	...
	// ret = led_init(); // TODO Tuan: delete
	// ERR_CHK(ret);

	// ret = button_handler_init(); // TODO Tuan: delete
	// ERR_CHK(ret);

	ret = channel_assign_check(); // TODO Tuan need to change - Done
	ERR_CHK(ret);
	...
	// if (board_rev.mask & BOARD_REVISION_VALID_MSK_MAX14690_PMIC)// TODO Tuan: delete
	// {
	// 	ret = pmic_init();
	// 	ERR_CHK(ret);
	...
	// if (board_rev.mask & BOARD_VERSION_VALID_MSK_SD_CARD)// TODO Tuan: delete
	// {
	// 	ret = sd_card_init();
	// 	if (ret != -ENODEV)
	// 	{
	...
	// ret = power_module_init();// TODO Tuan: delete
	// ERR_CHK(ret);
	...
	printk("ble init successfull\r\n");
	// ret = leds_set();// TODO Tuan: delete
	// ERR_CHK(ret);

	audio_system_init();// TODO Tuan: need to change
